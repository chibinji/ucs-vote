...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS "Voter" ("id" TEXT NOT NULL PRIMARY KEY, "computerNumber" TEXT NOT NULL, "csEmail" TEXT NOT NULL, "fullName" TEXT, "hasVoted" BOOLEAN NOT NULL DEFAULT false, "votedAt" DATETIME, "deviceTokenHash" TEXT, "deviceFingerprint" TEXT, "deviceLabel" TEXT, "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
  `CREATE TABLE IF NOT EXISTS "OtpCode" ("id" TEXT NOT NULL PRIMARY KEY, "voterId" TEXT NOT NULL, "codeHash" TEXT NOT NULL, "expiresAt" DATETIME NOT NULL, "attempts" INTEGER NOT NULL DEFAULT 0, "consumedAt" DATETIME, "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "OtpCode_voterId_fkey" FOREIGN KEY ("voterId") REFERENCES "Voter" ("id") ON DELETE CASCADE ON UPDATE CASCADE)`,
